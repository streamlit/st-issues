...
TITLE = "Streamlit Performance - Playwright Metrics Explorer"
...
keys_list = ", ".join([f"`{key}`" for key in json_file_calculations["phases"].keys()])
...
def check_long_animation_frames():
    if json_file_calculations["long_animation_frames"] > 0:
        return f"❌ Long Animation Frames detected: `{json_file_calculations['long_animation_frames']}ms`"
    return "✅ No Long Animation Frames detected"
